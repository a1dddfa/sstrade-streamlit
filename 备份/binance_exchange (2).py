...
# =========================
    ...
    # =========================
    # ⭐ Pinbar 扫描（USDT 永续）：1h K线，检索最新 N 根
    # =========================
    ...
    @staticmethod
    def _is_pinbar_candle(c: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        """判断单根 K 是否为 pinbar（按你的描述）。"""
        try:
            o = float(c.get("open"))
            h = float(c.get("high"))
            l = float(c.get("low"))
        ...
        # 长影线 > 2 * (实体 + 短影线)
        if long_wick <= 2.0 * (body + short_wick):
            return None

        direction = "short" if upper >= lower else "long"
        score = long_wick / max(1e-12, (body + short_wick))
        return {
            "direction": direction,
            "score": score,
            "body": body,
    ...
    def get_pinbar_pairs_usdt(
        ...
        """扫描所有 USDT 永续交易对的最近 lookback_bars 根 K，找 pinbar。"""
        now = time.time()

        cache_key = f"pinbar_usdt_{interval}_{int(lookback_bars)}"
        ...
        for sym in symbols:
            try:
                ...
                found = None
                found_idx = None  # 0 表示最新一根
                for i, candle in enumerate(reversed(kl)):
                    meta = self._is_pinbar_candle(candle)
                    if meta:
                        found = meta
                        found_idx = i
                        break
                ...
                results.append({
                    "symbol": sym,
                    "mode": found["direction"],   # short / long
                    "bar_index": int(found_idx),
                    "score": float(found["score"]),
                    "upper_wick": float(found["upper_wick"]),
                    "lower_wick": float(found["lower_wick"]),
                    "body": float(found["body"]),
                    "range": float(found["range"]),
                })
        ...
        results.sort(key=lambda x: (int(x.get("bar_index", 999)), -float(x.get("score", 0.0))))
