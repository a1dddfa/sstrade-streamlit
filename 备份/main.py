...
class TradingFramework:
    ...
    def _sync_dynamic_pairs(self):
        ...
        geN = self.exchange.get_top_reversal_pairs_usdt(
            top_n=300,
            min_abs_pct=self.min_abs_pct,
            fallback_top1_if_none=False,
            pos_threshold_short=self.pos_threshold_short,
            pos_threshold_long=self.pos_threshold_long,
            max_retrace_ratio=self.max_retrace_ratio,
            preselect_limit=self.preselect_limit,
            cache_ttl=60,
        ) or []

        current_pairs = set(self._dynamic_strategies.keys())

        # 当前运行中最大幅度（用 ticker 的 24h% 计算）
        current_max_abs = 0.0
        for p in current_pairs:
            try:
                t = self.exchange.get_ticker(p) or {}
                pct = float(t.get("priceChangePercent") or 0.0)
                current_max_abs = max(current_max_abs, abs(pct))
            except Exception:
                pass
            ...
            if current_pairs:
                ...
                for c in geN:
                    ...
                    if not sym or sym in desired:
                        continue
                    if float(c.get("abs_pct") or 0.0) > float(current_max_abs or 0.0):
                        desired.add(sym)
            else:
        ...
        # 最大不超过 3
        if len(desired) > max_burst:
            abs_map = {c.get("symbol"): float(c.get("abs_pct") or 0.0) for c in geN if c.get("symbol")}
            keep = sorted(list(desired), key=lambda s: abs_map.get(s, 0.0), reverse=True)[:max_burst]
            desired = set(keep)
